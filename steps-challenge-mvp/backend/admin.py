...
@router.post("/users/{user_id}/unapprove")
def unapprove_user(
    user_id: int,
    db: Session = Depends(get_db),
    admin: models.User = Depends(require_admin)
):
    """Revoke approval for a user (admin only)"""
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    if user.is_admin:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot unapprove an admin account")
    user.is_approved = False
    db.commit()
    return {"message": f"User {user.full_name} disabled successfully"}
